...
inline
unsigned short CalcCRCbyAlgorithm(unsigned char* pDataBuffer, int usDataLen)
{
...
int main() {
    ...
    unsigned char buffer[5] = { 0x00, 0x2A, 0x01 };//{ 0xAA, 0xCC, 0x00, 0x2A, 0x01, 0xAF, 0x60, 0xAA, 0xEE };
    result = CalcCRC(buffer, 3);
    printf("result = %x\n", result);

    result = CalcCRCbyAlgorithm(buffer, 3);
    printf("result2 = %x\n", result);

    unsigned char* pchar = (unsigned char*)&result;
